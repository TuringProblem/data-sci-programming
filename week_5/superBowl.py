...
# [year], [tag], [winning team] [Losing Team] [Score] [Location]
unique_team_list = []

#victory = {}
for superbowl in super_bowls:
    if superbowl[2] not in unique_team_list:
        unique_team_list.append(superbowl[2])
    if superbowl[3] not in unique_team_list:
        unique_team_list.append(superbowl[3])
...
for t in unique_team_list:
    team = [t, 0, 0]
    result.append(team) 

for superbowl in super_bowls:
    for team in result:
        if superbowl[2] == team[0]:
            team[1] += 1
        if superbowl[3] == team[0]:
            team[2] += 1
